...
const fallbackAnalyze = (
  ...
  if (!accepted) {
    return {
      ...
      coachingTip: "Try a short complete sentence so the counter staff can understand quickly.",
    };
  }
...
export const analyzeCheckpointWithAgent = async (
  ...
  if (!apiKey) {
    ...
    return fallbackAnalyze(checkpoint, userSpeech, profile);
  }
  ...
  try {
    const systemPrompt = [
      ...
      "If not accepted, feedback should gently guide the learner with an example answer.",
      "If accepted, feedback should acknowledge and continue naturally as check-in staff.",
